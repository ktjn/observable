...
Modelable 1.8.0 writes the canonical `registry-ids.lock` at the repository root when invoked with `modelable compile --registry-ids` (as both model scripts do), allocating stable small-integer ids for any `.mdl` semantic type declared `registry: true`. It is a tracked file, not gitignored — commit it alongside `.mdl` changes so ids are never silently reassigned. It is currently empty (`{}`) since no domain declares a `registry: true` type.

The `scripts/regenerate-models.sh` script runs both target compilations with the root registry lock, copies committed TypeScript/Rust artifacts, and applies the repository's documented TS import and Rust `From<&str>` patches automatically where required. Modelable remains a dev-time-only dependency; generated artifacts remain committed and are not produced by CI. After changing `.mdl` files:
1. Run `bash scripts/regenerate-models.sh` from the repository root.
2. Run `cargo fmt --all` to fix generated Rust formatting (also invoked by the script).
