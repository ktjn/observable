  ...
  return (
    <div>
      ...
      <label>Transport type<br />
        <select value={(transport.type as string) ?? ''} onChange={e => set({ type: e.target.value })}>
      ...
      {(transport.type === 'syslog_tcp' || transport.type === 'syslog_udp') && (
        <div style={{ marginTop: 12 }}>
          <label>Port<br />
            <input type="number" defaultValue={514}
      ...
      {transport.type === 'kafka' && (
        <div style={{ marginTop: 12 }}>
          ...
          <label style={{ marginTop: 8, display: 'block' }}>Topic<br />
            <input type="text" onChange={e => set({ topic: e.target.value })} />
          </label>
          <label style={{ marginTop: 8, display: 'block' }}>Consumer group<br />
            <input type="text" defaultValue="collectable"
      ...
      <button style={{ marginTop: 24 }} onClick={onNext} disabled={!transport.type}>
        Next →
      </button>
    </div>
  );
