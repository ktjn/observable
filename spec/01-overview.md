...
1. **OpenTelemetry first** — OTel is the primary ingestion and semantic model boundary. Vendor-specific agents are optional overlays, not core dependencies. The ingest gateway accepts OTLP over gRPC and HTTP plus the explicitly supported Prometheus Remote Write compatibility endpoint.
