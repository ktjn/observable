...
- **Implementation Plan Adherence:** All tasks must follow the active roadmap backlog in `ROADMAP.md`.
- **Finished Task Cleanup:** When a task is completed, update its status in `ROADMAP.md` and any relevant agent-context links. Do not reintroduce a public `archived/` directory of internal plan history.
...
8. **Retire completed tasks:** when the slice completes a roadmap item, update its status in `ROADMAP.md` and update active roadmap and `docs/agent-context.md` links in the same PR.
