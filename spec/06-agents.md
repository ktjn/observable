...
| Collectable | Independent compiled-mediator tool for legacy sources (syslog, log4j2, MQTT, webhooks, etc.) that cannot emit OTLP natively. Pipelines are defined in a web UI and compiled to static Rust binaries. Integrates with Observable via OTLP only. Developed in its own repository, [github.com/ktjn/collectable](https://github.com/ktjn/collectable); see [spec/16-collectable.md](16-collectable.md). |
