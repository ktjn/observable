...
| [ADR-029](ADR-029-simple-ir-shorthand.md) | Simple IR Shorthand & Deterministic Fallback | Proposed | Low-latency, deterministic shorthand for NLQ; prefix `/` bypasses LLM; serves as fallback when LLM is unavailable |
...
| [ADR-033](ADR-033-admin-service-extraction.md) | Admin Service Extraction | Proposed | Member management, token/API-key lifecycle, platform config, and usage reporting move out of query-api into a new admin-service for privilege isolation; auth-service stays read-only validator |
