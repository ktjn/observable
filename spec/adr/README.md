...
| [ADR-016](ADR-016-grafana-visualization-strategy.md) | Grafana Visualization Strategy | Accepted — not yet implemented | Grafana is supported as an optional visualization layer via datasource plugins; not the default UI |
...
| [ADR-018](ADR-018-rum-browser-mobile-ingestion.md) | RUM / Browser and Mobile Ingestion | Accepted — not yet implemented | Browser/mobile SDKs send to a dedicated RUM endpoint; not the same path as backend OTLP |
...
| [ADR-027](ADR-027-local-llm-backend.md) | LLM Backend Configuration (Unified Setup UI) | Accepted | Three-field Setup UI (API key, endpoint URL, model) works for all OpenAI-compatible providers; env vars override DB values; API key XOR-obfuscated at rest |
| [ADR-028](ADR-028-ingestion-token-environment-binding.md) | Ingestion Token — Per-Token Environment Binding | Accepted | Each ingestion token belongs to exactly one tenant and is scoped to exactly one environment; environment is stamped server-side at ingest from the token; client-supplied `deployment.environment` is not the authoritative source |
| [ADR-029](ADR-029-span-metrics-generation.md) | Span Metrics Generation | Accepted | In-stream RED-metrics aggregator in `stream-processor` derives span metrics from raw spans and emits them as OTLP-compliant metrics |
| [ADR-030](ADR-030-timestamp-representation.md) | Timestamp Representation — Unix Nanoseconds at Rest, Display Format at Presentation | Accepted | All timestamps stored and transported as Unix nanosecond integers; `fromUnixTimestamp64Nano()` and other conversions are forbidden in SQL SELECTs returned to clients; `formatTimestamp()` + `useTimeDisplay()` are the sole formatting points in the frontend |
| [ADR-031](ADR-031-global-tenant-environment-context.md) | Global Tenant and Environment Context in the Frontend UI | Accepted | Two `<Select>` dropdowns in the AppShell topbar backed by `useTenantContext()` React Context; all API calls receive `tenantId` as first parameter; default is the self-ingestion (system) tenant; auth filtering deferred |
| [ADR-032](ADR-032-modelable-type-mapping-adoption.md) | Adopt Modelable as the Type-Mapping Source of Truth | Accepted | `.mdl` files in `models/` are the canonical source for shared domain/wire types; generated Rust/TS artifacts are committed, not built in CI |
| [ADR-033](ADR-033-admin-service-extraction.md) | Admin Service Extraction | Accepted | Member management, token/API-key lifecycle, platform config, and usage reporting move out of query-api into a new admin-service for privilege isolation; auth-service stays read-only validator |
| [ADR-034](ADR-034-simple-ir-shorthand.md) | Simple IR Shorthand & Deterministic Fallback | Proposed | Low-latency, deterministic shorthand for NLQ; prefix `/` bypasses LLM; serves as fallback when LLM is unavailable. Renumbered from ADR-029 during 0.1 release cleanup to resolve a duplicate ADR number |
