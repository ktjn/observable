...
> `services/query-api/src/discovery.rs`). `archived/plans/2026-06-10-p9-s5-service-catalog-health-signals.md`
> closed the remaining gaps it scoped: real `active_alert_count` (SLO-linked alerts only — see
> that plan's documented limitation), real `latest_deployment`, and an SLO-burn-rate override for
> `health_state`. Still open: the fast-vs-slow burn red/yellow distinction, 30s background-poll
> refresh, and the open error-issue count (blocked on P9-S2). Not fully complete.
