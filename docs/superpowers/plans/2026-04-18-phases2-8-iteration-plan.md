...
- [x] **P3-S12a: Harden customer OTLP metric ingest readback**
  - Outcome: OTLP/HTTP JSON and gRPC metric conversion now preserve gauge, sum, and explicit histogram metadata consistently enough for readback. Metric series IDs are deterministic for a stable tenant, metric name, attributes, resource attributes, metric type, monotonicity, temporality, service, and environment, so repeated exports append points to the same queryable series. The smoke test now verifies `POST /v1/metrics` by waiting for `GET /v1/metrics?service=...` and then `GET /v1/metrics/{series_id}`.
  - Out of scope: Prometheus `remote_write`, PromQL, rollups/downsampling, exponential histogram support, summary support, and new dashboard UI.
...
**ADR-027 + P8-S6b implemented** (2026-04-29/2026-05-09) — ADR-027 records the decision to add vLLM as an opt-in local LLM backend for the NLQ pipeline. P8-S6b is the implementation slice: `VllmCaller` reusing `async-openai` with no auth key, the `PUT /v1/config/llm` unified endpoint for `llm_api_key`, `llm_url`, and `llm_model` fields, and an updated Setup page for configuration. This superseded the initial vLLM backend selector design. `spec/08-ai-ml.md §13.1` and `spec/adr/ADR-021-nl-query-layer.md` were updated in the same iteration to cross-reference ADR-027.
