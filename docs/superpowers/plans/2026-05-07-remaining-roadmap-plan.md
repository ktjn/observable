...
- [x] **Query Workbench**: Implement the Monaco-based multi-signal notebook for ad-hoc exploration. (COMPLETED 2026-06-01) Added `/workbench` with URL-backed notebook state, three fixed starter blocks for metrics/logs/traces, per-block NLQ/raw editing with Monaco, and browser/unit coverage for route, codec, shell, and accessibility.
...
- [x] **NLQ SQL Safety (RF-3)**: Implement field and type allowlisting in `query-api/src/sql_templates.rs` to prevent SQL injection and unsafe query shapes. (COMPLETED 2026-05-29) All active SQL template paths are covered: `catalog_field` and `group_by` fields validated by `validate_sql_identifier`; filter values validated by `build_filter_expr_checked` (numeric operators require f64, regex capped at 256 chars, strings escaped); log and trace facet fields restricted to explicit allowlists; metrics filter fields validated against `allowed_metric_filter_fields()` + schema registry. Unknown log attribute keys are safely routed to `JSONExtractString(attributes, '{escaped_key}')` — not treated as SQL identifiers. Two tests added: log filter unknown-field routing and log filter field injection-attempt escaping.
...
- [x] **P4-S6: Add production runbook set for one failure class** (COMPLETED 2026-05-22)
  ...
  - Next slice: P4-S9 boundary-focused security review.
...
- [x] **P4-S8: Run load, chaos, tenant-escape, and upgrade/rollback suites** (COMPLETED 2026-05-22)
