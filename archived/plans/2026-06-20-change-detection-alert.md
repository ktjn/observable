...
- `CreateRuleRequest` already has a `window_secs: Option<i64>` field used by the deadman type —
  reuse it for change-detection's window rather than adding a near-duplicate field; add only
  `baseline_offset_secs` and `threshold_percent` as new fields.

### Task 1: Design note + pure evaluation logic

Write a short design note at `docs/superpowers/specs/2026-06-20-change-detection-alert-design.md`
(mirroring `docs/superpowers/specs/2026-06-18-p12-s3-deadman-alert-design.md`'s structure) fixing
the condition JSON shape from Global Constraints, the percent-change formula and directionality
decision (bidirectional), and the divide-by-zero rule.

In `services/alert-evaluator/src/evaluator.rs`:
- Add a `ChangeDetectionCondition` struct (alongside `DeadmanCondition`, ~line 56) with fields
  `metric_name: String`, `window_secs: i64`, `baseline_offset_secs: i64`, `threshold_percent: f64`.
- Add `pub fn evaluate_change_detection(current_avg: f64, baseline_avg: f64, condition: &ChangeDetectionCondition) -> EvalResult`
  — a pure function, no I/O, implementing the bidirectional percent-change formula and the
  divide-by-zero guard from Global Constraints.
- Add unit tests in the same file (`#[cfg(test)] mod tests`, matching the existing
...
Verify with `cargo test -p alert-evaluator` (unit tests only at this point, no ClickHouse needed)
and `cargo fmt --all`.

### Task 2: `alert-evaluator` — ClickHouse window-average fetch + evaluation loop

Depends on Task 1's `ChangeDetectionCondition` and `evaluate_change_detection`.

In `services/alert-evaluator/src/evaluator.rs`:
- Add a `WindowAvgRow` struct (`#[derive(clickhouse::Row, serde::Deserialize)]`, alongside
  `LatestPointRow`/`LastSeenRow`) with an `avg_value: Option<f64>` field — ClickHouse's `avg()`
  returns NULL for an empty window; handle `None` as "skip this rule", do not coerce to 0.0.
- Add `pub async fn eval_change_detection_rules(db: &sqlx::PgPool, ch: &clickhouse::Client) -> anyhow::Result<()>`,
  placed after `eval_deadman_rules` (~line 543), following that function's exact structural
  pattern: `SELECT rule_id, tenant_id, name, condition, severity, for_duration_secs, notification_channels, auto_trigger_incident, auto_trigger_delay_secs, runbook_url FROM alert_rules WHERE alert_type = 'change_detection' AND silenced = false`,
  parse condition JSON (skip + `tracing::warn!` on parse failure), then for each rule run two
  ClickHouse queries against `observable.metric_points`:
  ```sql
  ...
  ```
  bound once for the current window (`now - window_secs` to `now`) and once for the baseline window
  (`now - baseline_offset_secs - window_secs` to `now - baseline_offset_secs`). Convert seconds to
  nanosecond unix timestamps the same way the existing deadman/threshold queries do (inspect
  `eval_threshold_rules` and `eval_deadman_rules` for the existing conversion pattern and reuse it
  rather than inventing a new one).
- On `None`/missing average for either window, skip the rule with `tracing::warn!` (same pattern as
  threshold's "skipping rule: metric fetch failed").
...
Test: extend or add a file under `services/alert-evaluator/tests/` mirroring the existing
Testcontainers pattern used for deadman or SLO burn-rate evaluation (ClickHouse container setup +
Postgres pool + seeded `alert_rules`/`metric_points` rows). Add a case seeding metric points in two
distinct time ranges and asserting a firing transitions `alert_firings` to `active`, plus a
within-threshold case that stays `ok`.

Verify with `cargo test -p alert-evaluator` (requires Docker for the Testcontainers test) and
`cargo fmt --all`.

### Task 3: `query-api` — CRUD support for change-detection rules

Independent of Tasks 1-2 (separate service); can be implemented in parallel but review/merge after
confirming the condition JSON shape matches Task 1's design note.

In `services/query-api/src/alerts.rs`:
...
- `CreateRuleRequest` (line 91-103): add `baseline_offset_secs: Option<i64>` and
  `threshold_percent: Option<f64>` as new fields (reuse the existing `window_secs: Option<i64>`
  field already used by deadman — see Global Constraints).
- `create_alert_rule` (line 230-357): add a `"change_detection" =>` match arm following the
  `"deadman"` arm's structure — validate `metric_name` non-empty, `window_secs`,
  `baseline_offset_secs` present and positive, `threshold_percent` present/finite/non-negative,
  build the condition JSON per Task 1's design note shape, insert with
  `alert_type = 'change_detection'`, return an `AlertRuleItem` with `metric_name`,
  `operator: "change_detection"`, `threshold: threshold_percent`.
...
Verify with `cargo test -p query-api` (requires Docker for Testcontainers) and `cargo fmt --all`.

### Task 4: Frontend — alert-type selector and form fields

Depends on Task 3's `CreateRuleRequest` field names (`baseline_offset_secs`, `threshold_percent`,
reused `window_secs`).

- `apps/frontend/src/api/alerts.ts`: widen the `CreateRuleRequest` TS interface with
  `baseline_offset_secs?: number` and `threshold_percent?: number`; widen any `alert_type` union
  type to include `"change_detection"`.
- `apps/frontend/src/features/alerts/AlertsPage.tsx`:
  - Widen `formAlertType`'s type (line 41) to `"threshold" | "deadman" | "change_detection"` and
    add state vars for the change-detection metric name, baseline offset seconds, and threshold
    percent (mirror `formServiceName`/`formWindowSecs`'s declaration pattern; reuse
    `formWindowSecs` for the change-detection window since the condition shape shares that field
    with deadman).
  - Add a third `<SelectOption value="change_detection">` to the alert-type `<Select>` (line
    254-261), label "Change detection".
  ...
  - Extend `handleCreateSubmit` (line 110-154) with a `change_detection` branch before the existing
    `deadman`-then-`threshold` checks, validating the numeric fields are positive finite numbers
    and calling `createMutation.mutate` with `alert_type: "change_detection"`, `metric_name`,
    `window_secs`, `baseline_offset_secs`, `threshold_percent`.
...
Verify with `npm run typecheck` and `npm run test` (Vitest) in `apps/frontend/`.

### Task 5: Spec sync and roadmap closeout

Depends on Tasks 1-4 being complete (describes what was actually built).
...
- Add a closure entry to `docs/agent-context.md` matching the existing closeout style (see the
  deadman closure entry for the pattern).
- Move this plan file to `archived/plans/`.

No automated verification beyond confirming the moved/edited files render correctly — this is a
docs-only task.
