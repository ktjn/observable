...
#   1. Deploy ingest-gateway-canary alongside stable via Helm values override.
#   2. Run three automated analysis gates against the canary Service:
...
#   3. Promote on pass: upgrade stable release to new tag, remove canary.
#   4. Revert on failure: remove canary; stable release is unchanged.
#
# Usage:
