...
function mockIncidents(items: IncidentItem[]) {
  vi.mocked(useRuntime).mockReturnValue({
    incidents: { list: vi.fn(async () => ({ items })), get: vi.fn() },
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(useRuntime).mockReturnValue({
    incidents: { list: vi.fn(async () => ({ items: sampleIncidents })), get: vi.fn() },
  } as unknown as RuntimeApi);
});
