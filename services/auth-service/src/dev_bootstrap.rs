...
// Ensures admin@dev.observable has a tenant_admin role on dev-tenant after Zitadel login.
// The user row is created by the OIDC callback on first login; this pre-seeds
// the role assignment so login -> tenant works immediately.
//
