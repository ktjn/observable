...
// tenant and dev-tenant after Zitadel login.  The user row is created by the
...
pub async fn seed_dev_admin_role(pool: &PgPool, dev_admin_email: &str) -> Result<()> {
    ...
    if let Some(uid) = user_id {
        for tenant_str in [OBSERVABLE_TENANT_ID, DEV_TENANT_ID] {
        ...
        tracing::info!(
            ...
            "dev admin role ensured on observable and dev-tenant"
        );
    } else {
        tracing::info!(
            email = dev_admin_email,
