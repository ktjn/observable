...
impl AuthServiceMetrics {
    pub fn new() -> Self {
        ...
        .expect("create auth_service_http_requests_total");

        let http_request_duration_seconds = HistogramVec::new(
            histogram_opts!(
                "auth_service_http_request_duration_seconds",
                "HTTP request duration in seconds for auth-service",
                linear_buckets(0.005, 0.005, 20).expect("valid histogram buckets")
            ),
            &["method", "status"],
        )
        .expect("create auth_service_http_request_duration_seconds");

        let http_in_flight_requests = IntGauge::with_opts(opts!(
            "auth_service_http_in_flight_requests",
            "Current in-flight HTTP requests handled by auth-service"
        ))
        .expect("create auth_service_http_in_flight_requests");
...
impl Default for AuthServiceMetrics {
    fn default() -> Self {
        Self::new()
    }
}
...
pub async fn metrics(State(state): State<OidcState>) -> Response {
...
pub async fn readyz(State(state): State<OidcState>) -> StatusCode {
    match state.db.acquire().await {
        Ok(_) => StatusCode::OK,
        Err(e) => {
            tracing::warn!(error = %e, "auth-service readiness postgres check failed");
