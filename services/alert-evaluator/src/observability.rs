...
impl AlertEvaluatorMetrics {
    pub fn new() -> Self {
        let registry = Registry::new();

        let http_requests_total = IntCounterVec::new(
            opts!(
                "alert_evaluator_http_requests_total",
                "Total HTTP requests handled by alert-evaluator"
            ),
            &["method", "status"],
        )
        .expect("create alert_evaluator_http_requests_total");

        let http_request_duration_seconds = HistogramVec::new(
            histogram_opts!(
                "alert_evaluator_http_request_duration_seconds",
                "HTTP request duration in seconds for alert-evaluator",
                linear_buckets(0.005, 0.005, 20).expect("valid histogram buckets")
            ),
            &["method", "status"],
        )
        .expect("create alert_evaluator_http_request_duration_seconds");

        let http_in_flight_requests = IntGauge::with_opts(opts!(
            "alert_evaluator_http_in_flight_requests",
            "Current in-flight HTTP requests handled by alert-evaluator"
        ))
        .expect("create alert_evaluator_http_in_flight_requests");

        let evaluations_total = IntCounterVec::new(
            opts!(
                "alert_evaluator_evaluations_total",
                "Total alert evaluation cycles"
            ),
            &["result"],
        )
        .expect("create alert_evaluator_evaluations_total");

        let notifications_total = IntCounterVec::new(
            opts!(
                "alert_evaluator_notifications_total",
                "Total alert notifications sent"
            ),
            &["result"],
        )
        .expect("create alert_evaluator_notifications_total");
        ...
        registry
            ...
            .expect("register evaluations_total");
        registry
...
impl Default for AlertEvaluatorMetrics {
    fn default() -> Self {
        Self::new()
    }
}
